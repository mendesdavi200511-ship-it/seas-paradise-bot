...
async def criar_tabelas():
    ...
    async with db.acquire() as conn:
        ...
        # =================================================
        # REPUTAÇÃO REATIVA — personagem/facções/procura
        # Sem FK para fichas: a reputação histórica sobrevive à morte.
        # =================================================
        await conn.execute("""
        ...
        """)
        await conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_noticias_mundo_criado
            ON noticias_mundo(publicado, criado_em DESC);
        """)
...
# =========================================================
# MUNDO REATIVO — PROCURA / FACÇÕES / JORNAL
# =========================================================
...
async def aplicar_impacto_reputacao(
    ...
    delta_marinha = (g * 2 if marinha_sabe else 0) + (g * 3 if hostil_marinha else 0)
    delta_governo = (g * 2 if governo_sabe else 0) + (g * 3 if hostil_governo else 0)
    delta_piratas = (g * 2 if piratas_sabem else 0) + (-g if hostil_piratas else 0)
...
# =========================================================
# ESTADO PERSISTENTE DO JOGADOR — compatibilidade do Narrador
# =========================================================
async def _garantir_tabela_estado_jogador():
    db = get_pool()
    await db.execute("""
        CREATE TABLE IF NOT EXISTS estados_jogador (
            user_id BIGINT PRIMARY KEY,
            estado TEXT NOT NULL DEFAULT 'livre',
            custodia TEXT,
            restricoes TEXT,
            combate BOOLEAN NOT NULL DEFAULT FALSE,
            atualizado_em TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        );
    """)

async def buscar_estado_jogador(user_id):
    await _garantir_tabela_estado_jogador()
    db = get_pool()
    return await db.fetchrow("SELECT * FROM estados_jogador WHERE user_id=$1;", user_id)

async def atualizar_estado_jogador(user_id, estado=None, custodia=None, restricoes=None, combate=None):
    await _garantir_tabela_estado_jogador()
    db = get_pool()
    atual = await buscar_estado_jogador(user_id)
    estado_final = estado if estado is not None else (atual['estado'] if atual else 'livre')
    custodia_final = custodia if custodia is not None else (atual['custodia'] if atual else None)
    restricoes_final = restricoes if restricoes is not None else (atual['restricoes'] if atual else None)
    combate_final = bool(combate) if combate is not None else (bool(atual['combate']) if atual else False)
    return await db.fetchrow("""
        INSERT INTO estados_jogador(user_id, estado, custodia, restricoes, combate, atualizado_em)
        VALUES($1,$2,$3,$4,$5,CURRENT_TIMESTAMP)
        ON CONFLICT(user_id) DO UPDATE SET
            estado=EXCLUDED.estado, custodia=EXCLUDED.custodia,
            restricoes=EXCLUDED.restricoes, combate=EXCLUDED.combate,
            atualizado_em=CURRENT_TIMESTAMP
        RETURNING *;
    """, user_id, estado_final, custodia_final, restricoes_final, combate_final)

async def resetar_estado_jogador(user_id):
    await _garantir_tabela_estado_jogador()
    db = get_pool()
    await db.execute("DELETE FROM estados_jogador WHERE user_id=$1;", user_id)
