...
async def criar_tabelas():
    ...
    async with db.acquire() as conn:
        ...
        # =================================================
        # HISTÓRICO PERSISTENTE DO NARRADOR
        # Mantém continuidade das cenas mesmo após reinício do bot.
        # =================================================

        await conn.execute("""
            CREATE TABLE IF NOT EXISTS historico_narrador (
        ...
        await conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_historico_narrador_user
            ON historico_narrador(user_id, criado_em DESC);
        """)
...
async def registrar_npc(
    ...
    return await db.fetchrow(
        ...
        ON CONFLICT (nome_chave) DO UPDATE SET
            nome = EXCLUDED.nome,
            localizacao = COALESCE(EXCLUDED.localizacao, npcs_mundo.localizacao),
            faccao = COALESCE(EXCLUDED.faccao, npcs_mundo.faccao),
            atualizado_em = CURRENT_TIMESTAMP
        RETURNING *;
        """,
...
def perfil_npc_valido(perfil):
    """Rejeita perfis antigos/provisórios para que sejam regenerados automaticamente."""
    if not isinstance(perfil, dict):
        return False
    obrigatorias = {
        "nome", "titulo", "classificacao", "raca", "faccao",
        "forca", "resistencia", "velocidade", "akuma", "haki",
        "estilo", "arma", "publico", "combate"
    }
    if not obrigatorias.issubset(perfil):
        return False

    texto = " ".join(str(perfil.get(k, "")) for k in (
        "titulo", "classificacao", "raca", "faccao", "akuma",
        "haki", "estilo", "arma", "publico", "combate"
    )).casefold()
    marcadores_provisorios = (
        "identificação pendente", "identificacao pendente",
        "perfil provisório", "perfil provisorio",
        "a confirmar", "não confirmada", "nao confirmada",
        "não confirmado", "nao confirmado"
    )
    if any(m in texto for m in marcadores_provisorios):
        return False

    try:
        valores = [int(perfil[k]) for k in ("forca", "resistencia", "velocidade")]
    except (TypeError, ValueError):
        return False
    return all(0 <= v <= 50000 for v in valores)


async def buscar_perfil_npc(nome):
    """Retorna apenas perfil mecânico COMPLETO. Provisórios antigos são tratados como inexistentes."""
    import json
    npc = await buscar_npc(nome)
    ...
    return perfil if perfil_npc_valido(perfil) else None


async def salvar_perfil_npc(nome, perfil):
    """Persiste perfil válido; substitui automaticamente perfis provisórios herdados."""
    import json
    if not perfil_npc_valido(perfil):
        raise ValueError(f"Perfil mecânico inválido/incompleto para {nome}.")
    npc = await registrar_npc(nome)
    db = get_pool()
    async with db.acquire() as conn:
        async with conn.transaction():
            ...
            if atual["dados"]:
                try:
                    ...
                    dados = {}
            existente = dados.get("perfil_combate")
            if perfil_npc_valido(existente):
                return existente
...
async def registrar_historico_narrador(user_id, channel_id, acao, narracao, localizacao=None):
    db = get_pool()
    return await db.fetchrow(
        """
        INSERT INTO historico_narrador (user_id, channel_id, localizacao, acao, narracao)
        VALUES ($1,$2,$3,$4,$5)
        RETURNING *;
        """,
        user_id, channel_id, localizacao, acao, narracao
    )


async def buscar_historico_narrador(user_id, limite=12):
    """Últimos turnos do personagem, inclusive entre canais, em ordem cronológica."""
    db = get_pool()
    rows = await db.fetch(
        """
        SELECT * FROM historico_narrador
        WHERE user_id=$1
