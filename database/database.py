...
def perfil_npc_valido(perfil):
    """Recusa perfis incompletos e os placeholders antigos de identificação pendente."""
    if not isinstance(perfil, dict) or any(c not in perfil for c in CAMPOS_PERFIL_NPC):
        return False
    ...
    try:
        atributos = [int(perfil[c]) for c in ("forca", "resistencia", "velocidade")]
    except (TypeError, ValueError):
        return False
    if any(v < 0 or v > 50000 for v in atributos):
        return False

    marcador = " ".join(str(perfil.get(c, "")).casefold() for c in (
        "titulo", "classificacao", "raca", "faccao", "estilo", "arma", "publico"
    ))
    placeholders = (
        "identificação pendente", "identificacao pendente", "perfil provisório",
        "perfil provisorio", "não confirmada", "nao confirmada", "a confirmar",
    )
    if any(x in marcador for x in placeholders):
        return False

    # Assinatura exata do fallback antigo que gerava status genérico 1000/1000/1000.
    if atributos == [1000, 1000, 1000] and any(x in marcador for x in ("independente", "desconhecido", "provis")):
        return False
    return True


async def buscar_perfil_npc(nome):
    """Retorna somente perfil mecânico persistido que passou pela validação atual."""
    import json
    npc = await buscar_npc(nome)
...
async def salvar_perfil_npc(nome, perfil):
    """Mantém perfil válido já salvo; substitui automaticamente perfil antigo inválido/provisório."""
    import json
    if not perfil_npc_valido(perfil):
        raise ValueError(f"Perfil mecânico inválido para {nome}.")
