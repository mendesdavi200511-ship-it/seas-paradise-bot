...
async def resetar_ficha_por_morte(user_id):
    """Reset total do personagem após morte narrativa confirmada.

    Remove a ficha e tudo que depende dela por FK/CASCADE, limpa as rolagens
    de criação para permitir um personagem realmente novo e desfaz vínculos
    de recrutamento presos ao mesmo user_id. As memórias dos NPCs são
    preservadas como história do mundo/personagem falecido.
    """
    db = get_pool()

    async with db.acquire() as conn:
        async with conn.transaction():
            # O novo personagem do mesmo usuário não herda tripulação do morto.
            await conn.execute(
                """
                UPDATE npcs_mundo
                SET recrutado_por = NULL,
                    disponivel = CASE WHEN status = 'morto' THEN FALSE ELSE TRUE END,
                    atualizado_em = CURRENT_TIMESTAMP
            ...
            # Não possui FK para fichas, então precisa ser limpo explicitamente.
            await conn.execute(
                "DELETE FROM rolagens_criacao WHERE user_id = $1;",
                user_id
            )
            ...
            # são removidos automaticamente por ON DELETE CASCADE.
