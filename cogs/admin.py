...
def _nome_item_catalogo(item):
    """Normaliza item de catálogo sem assumir o formato de data.skills."""
    if isinstance(item, str):
        return item
    if isinstance(item, dict):
        for chave in ("nome", "name", "label", "titulo", "title"):
            valor = item.get(chave)
            if valor:
                return str(valor)
        # Formato comum: {"Ittoryu": {...}}
        if len(item) == 1:
...
def nomes_catalogo(categoria):
    """Retorna somente nomes válidos, inclusive quando ESTILOS é lista de dicts."""
    catalogo = CATEGORIAS_CATALOGO.get(categoria)
    if isinstance(catalogo, dict):
        nomes = [str(nome) for nome in catalogo.keys()]
    elif isinstance(catalogo, (list, tuple, set)):
        nomes = []
        for item in catalogo:
            nome = _nome_item_catalogo(item)
            if nome:
                nomes.append(nome)
    else:
        nomes = []
    # Discord limita label/value de SelectOption a 100 caracteres.
    return [nome[:100] for nome in nomes if nome]
...
def obter_emoji(categoria, nome):
    ...
    if isinstance(catalogo, dict):
        ...
        for item in catalogo:
            if _nome_item_catalogo(item) == nome:
                dados = item
                break

    if isinstance(dados, dict):
        emoji = dados.get("emoji") or dados.get("icon")
        if emoji:
            return str(emoji)
    return "🥋" if categoria == "estilo" else "📚"
...
class ItemCatalogoSelect(
    ...
    def __init__(
        ...
        for nome in itens[:25]:
            ...
            # Não passa emojis vindos do catálogo para o SelectOption.
            # Um emoji inválido/customizado pode impedir o catálogo inteiro
            # de abrir. Classe, Estilo e Profissão usam o mesmo caminho seguro.
            opcoes.append(
