...
# =====================================================
    ...
    async def resetarficha(
        ...
        # Localiza a central do personagem ANTES de apagar a ficha.
        thread = None
        canal = None

        if guild is not None:
            # Caso o reset seja executado dentro da própria central.
            if (
                isinstance(ctx.channel, discord.Thread)
                and ctx.channel.parent_id == CANAL_CRIACAO_ID
            ...
            # Cache de tópicos ativos da guild.
            if thread is None:
            ...
            # Obtém o canal oficial mesmo se ele não estiver no cache.
            try:
            ...
            # Consulta também os tópicos ativos pela API.
            if thread is None:
                try:
                    ativas = await guild.active_threads()
                    for candidata in ativas:
                        if (
                            candidata.parent_id == CANAL_CRIACAO_ID
                            and marcador in candidata.name
                        ):
            ...
            # Tópicos privados arquivados, caso o modelo mude no futuro.
            if thread is None and canal is not None:
                try:
                    async for candidata in canal.archived_threads(
                        limit=None,
                        private=True,
                        joined=False
                    ):
        ...
        resultado = await deletar_ficha(membro.id)
        await resetar_rolagem_criacao(membro.id)
        criando.pop(membro.id, None)

        if resultado == "DELETE 0":
            await ctx.send(
                f"❌ {membro.mention} não possui ficha.",
                delete_after=TIMEOUT_PAINEL
            )
            return
        ...
        if thread is not None:
            try:
                ...
                erro_thread = (
                    "o bot não tem permissão **Gerenciar Tópicos** "
                    "para apagar a central do personagem"
                )
            except discord.NotFound:
                thread_apagada = True
            except discord.HTTPException as erro:
                erro_thread = f"Discord recusou a exclusão: {erro}"
        else:
            erro_thread = f"tópico `{marcador}` não encontrado"

        # Se o comando foi usado dentro da central apagada, não tenta responder nela.
        if (
            thread_apagada
            and isinstance(ctx.channel, discord.Thread)
            and thread is not None
            and ctx.channel.id == thread.id
        ):
            return

        mensagem = f"🗑️ Ficha de {membro.mention} resetada."
        if thread_apagada:
            mensagem += "\n🧹 Central do personagem apagada."
        elif erro_thread:
            mensagem += f"\n⚠️ Ficha apagada, mas {erro_thread}."
