...
# O canal identifica ONDE a ação está sendo tentada. Ele NÃO altera a localização
# persistente do personagem. Canais administrativos/comuns são ignorados.
...
class Narrador(commands.Cog):
    ...
    async def validar_local_da_acao(self, ctx):
        """Validação mecânica obrigatória ANTES da IA.

        O canal representa o local em que a ação está sendo tentada, mas NUNCA
        altera sozinho a posição persistente do personagem. Assim, trocar de
        canal não teleporta o player e uma ação inválida não chega ao Narrador,
        nem alimenta memória, evento, reputação ou jornal.
        """
        local_canal = self.localizacao_do_canal(ctx)
        if not local_canal:
            return False, (
                "❌ `!acao` só pode ser usada em um canal de localização do RP. "
                "Este canal não representa uma área válida do mundo."
            )

        atual = await buscar_localizacao_jogador(ctx.author.id)

        # Compatibilidade com personagens antigos: somente quando ainda NÃO há
        # posição persistida, a primeira cena estabelece o ponto inicial.
        if not atual or not atual["localizacao"]:
            await definir_localizacao_jogador(ctx.author.id, local_canal)
            print(
                f"🧭 Local inicial de {ctx.author.id} estabelecido em "
                f"{local_canal} pelo canal #{ctx.channel.name}."
            )
            return True, None

        local_atual = str(atual["localizacao"]).strip()
        if local_atual.casefold() != local_canal.casefold():
            area = f" • {atual['area']}" if atual["area"] else ""
            return False, (
                f"🚫 **Você não está neste local.**\n"
                f"📍 Localização atual: **{local_atual}**{area}\n"
                f"🗺️ Este canal representa **{local_canal}**.\n"
                "Conclua um deslocamento válido no mundo antes de agir aqui. "
                "Trocar de canal não move o personagem."
            )
    ...
    def _normalizar_perfil_gerado(self, nome, bruto):
        """Aceita JSON puro ou JSON cercado por texto/markdown e devolve um perfil válido."""
        texto = (bruto or "").strip()
        if not texto:
            return None

        # Remove fences sem depender de o modelo obedecer 100% ao formato pedido.
        ...
        # Campos textuais ausentes não derrubam o bot. O perfil pode ser completado
        # sem inventar poder: desconhecido/não confirmado é preferível a falhar.
        defaults = {
            "nome": str(nome).strip(),
            "titulo": "Não confirmado",
            "classificacao": "Não confirmada",
            "raca": "Não confirmada",
            "faccao": "Independente",
            "akuma": "Nenhuma/Não confirmada",
            "haki": "Não confirmado",
            "estilo": "A confirmar",
            "arma": "A confirmar",
            "publico": "Informações públicas ainda não confirmadas.",
            "combate": "Usar apenas capacidades confirmadas no contexto e na cronologia atual.",
        }
        for campo, valor in defaults.items():
            if obj.get(campo) in (None, ""):
                obj[campo] = valor

        # Os três atributos são a única parte mecânica que precisa obrigatoriamente
        # vir calibrada. Se um deles não puder ser convertido, pedimos nova geração.
        for campo in ("forca", "resistencia", "velocidade"):
            try:
                obj[campo] = max(0, min(50000, int(float(str(obj[campo]).replace(",", ".")))))
            except (KeyError, TypeError, ValueError):
                return None
        return obj

    async def gerar_perfil_npc_automatico(self, nome, contexto=""):
        """Gera UMA VEZ qualquer NPC individual e persiste o resultado no PostgreSQL."""
        canonico = self.nome_canonico(nome)

        if self.entidade_coletiva_ou_nao_individual(canonico):
            return None

        # Perfis calibrados manualmente continuam tendo prioridade absoluta.
        ...
        # Se já nasceu antes, nunca sorteia/recria: usa exatamente o perfil persistido.
        ...
        entrada_base = f"""
NPC INDIVIDUAL: {nome}
UNIVERSO: One Piece — Sea's Paradise.
CONTEXTO/Fase do RP: {contexto or "Use somente a fase coerente com o estado atual do Sea's Paradise."}

Identifique primeiro quem é o personagem. Nomes curtos ou ambíguos (por exemplo Pica) devem ser interpretados como personagem de One Piece quando houver personagem canônico correspondente.
Crie o perfil mecânico persistente deste NPC.
Se for CANÔNICO, respeite identidade, raça, facção, poderes, armas, estilo e cronologia confirmados. Não dê poderes futuros, despertar futuro, Haki futuro, arma errada, veículo errado ou técnica inventada.
Se uma informação canônica realmente não estiver confirmada, use "Não confirmado" em vez de inventar.
Se for ORIGINAL, crie um kit coerente com função, local e contexto.

ESCALA DO RP: 0 a 50.000 para Força, Resistência e Velocidade/Agilidade.
Calibre por ameaça: civil comum dezenas; combatentes fracos centenas; East Blue relevante milhares; elite da Grand Line dezenas de milhares; topo mundial perto de 50.000.

Retorne SOMENTE um objeto JSON. Chaves obrigatórias:
{{"nome":"", "titulo":"", "classificacao":"", "raca":"", "faccao":"", "forca":0, "resistencia":0, "velocidade":0, "akuma":"", "haki":"", "estilo":"", "arma":"", "publico":"", "combate":""}}
""".strip()

        ultimo_bruto = ""
        # Três tentativas locais impedem que um JSON malformado derrube !status.
        for tentativa in range(3):
            reforco = "" if tentativa == 0 else (
                "\nATENÇÃO: a resposta anterior não pôde ser lida. "
                "Responda novamente com APENAS JSON válido, começando em { e terminando em }. "
                "Não use markdown nem explicações."
            )
            resposta = await self.client.responses.create(
                model=MODELO_NARRADOR,
                instructions=(
                    "Você cria perfis mecânicos de personagens para Sea's Paradise. "
                    "Reconheça personagens canônicos de One Piece mesmo quando o nome for curto. "
                    "Priorize cânone e cronologia e nunca invente habilidade canônica não confirmada. "
                    "Sua saída deve ser somente JSON válido."
                ),
                input=entrada_base + reforco,
                max_output_tokens=900,
            )
            ultimo_bruto = (resposta.output_text or "").strip()
            perfil = self._normalizar_perfil_gerado(nome, ultimo_bruto)
            if perfil:
                # Persiste antes de expor ao restante do sistema. Em corrida, o primeiro salvo vence.
        ...
        raise RuntimeError(
            f"Não foi possível obter um perfil mecânico estruturado para {nome} após 3 tentativas."
        )
    ...
    async def contexto_mundo(self, acao, user_id=None, personagem_ativo=None):
        ...
        # NUNCA recupere localização usando apenas user_id.
        # O mesmo usuário pode ter tido personagens anteriores já mortos.
        # Personagem novo começa com o próprio estado; a história antiga permanece no mundo,
        # mas não vira estado físico/controle do personagem atual.

        partes = [
            "PERSONAGEM ATIVO CONTROLADO PELO JOGADOR: " + (personagem_ativo or "não definido"),
            "LOCALIZAÇÃO DO PERSONAGEM ATIVO: " + (local_nome or "desconhecida/não estabelecida"),
            "ÁREA DO PERSONAGEM ATIVO: " + (area_player or "não estabelecida"),
            "REGRA: localização declarada pelo jogador não substitui o estado persistente.",
            "REGRA DE IDENTIDADE: eventos e memórias de personagens antigos são HISTÓRIA DO MUNDO. "
            "Nunca trate um nome histórico como o personagem ativo e nunca transfira prisão, ferimentos, "
            "inventário, posição, combate ou controle de um personagem morto para o atual."
        ]
        for npc in citados[:8]:
            ...
            if memorias:
                linhas.append("Memórias históricas do NPC (podem envolver personagens antigos/mortos):")
                for m in memorias:
                    origem = m["personagem_nome"] or "personagem não identificado"
                    linhas.append(f"- [sobre {origem}] {m['resumo']}")
            partes.append("\n".join(linhas))
        if len(partes) == 3:
            partes.append("NPCS CITADOS: nenhum NPC persistente/catalogado reconhecido.")

        # Reação sistêmica ao PERSONAGEM ATIVO: procura, relações e reconhecimento.
        if personagem_ativo:
            rep = await buscar_reputacao_mundo(personagem_ativo)
            if rep:
                partes.append(
                    "REAÇÃO ATUAL AO PERSONAGEM:\n"
                    f"- Procurado: {'sim' if rep['procurado'] else 'não'}\n"
                    f"- Recompensa: ฿ {int(rep['recompensa']):,}\n"
                    f"- Nível de ameaça estatal: {rep['nivel_ameaca']}/5\n"
                    f"- Relação Marinha: {rep['marinha']}\n"
                    f"- Relação Governo: {rep['governo']}\n"
                    f"- Relação Piratas: {rep['piratas']}\n"
                    f"- Relação Civis: {rep['civis']}\n"
                    "REGRA: valores negativos indicam hostilidade; positivos indicam reconhecimento/favor. "
                    "Reaja de forma proporcional e somente quando a facção/local plausivelmente conhecer a identidade."
                )

        noticias = await buscar_noticias_mundo(6)
        if noticias:
            partes.append(
                "NOTÍCIAS QUE PODEM ESTAR CIRCULANDO NO MUNDO:\n" +
                "\n".join(
                    f"- [{n['alcance']}] {n['manchete']}" for n in noticias
                ) +
                "\nREGRA: notícia não significa conhecimento instantâneo em todo lugar; considere alcance e tempo plausível."
            )
...
""".strip()
    ...
    async def gerar_narracao(self, ctx, acao, ficha, especializacoes):
        mundo = await self.contexto_mundo(acao, ctx.author.id, ficha["nome"])
...
REAÇÃO DO MUNDO:
- Se houver procura/hostilidade conhecida, Marinha/Governo podem identificar, vigiar, abordar, perseguir, prender ou escalar força conforme ameaça e contexto.
- Piratas podem respeitar, temer, hostilizar, recrutar, desafiar ou evitar conforme reputação e fatos conhecidos.
- Civis podem reconhecer, ajudar, denunciar, fugir, admirar ou rejeitar conforme fatos públicos.
- Recompensa, fama e reconhecimento NÃO surgem porque o jogador pediu; surgem das consequências persistidas.
- Outros jogadores compartilham o mesmo mundo: acontecimentos publicados/persistidos podem afetar cenas futuras deles.

IDENTIDADE ATIVA — REGRA ABSOLUTA
O personagem que o jogador controla NESTA ação é EXCLUSIVAMENTE {ficha["nome"]}.
O Discord user_id identifica a conta, não uma identidade eterna de personagem.
Qualquer outro nome encontrado em eventos, memórias de NPC ou consequências do mundo é histórico,
a menos que a ficha ativa tenha exatamente aquele nome.
Nunca continue uma prisão, ferimento, morte, localização, combate ou ação de personagem anterior
como se pertencesse a {ficha["nome"]}.
...
""".strip()
    ...
    async def acao(self, ctx, *, texto: str):
        ...
        async with locks[cena_id]:
            # Revalida a identidade ativa dentro do lock.
            # Se a ficha morreu/resetou enquanto a ação aguardava, ela NÃO pode continuar.
            ficha = await buscar_ficha(ctx.author.id)
            if not ficha:
                await ctx.send(
                    "❌ Você ainda não possui uma ficha ativa. "
                    "Crie um novo personagem para usar `!acao`."
                )
                return

            # PORTÃO MECÂNICO GLOBAL: precisa passar ANTES de IA, histórico,
            # memórias, eventos, reputação e jornal. O canal jamais teleporta.
            local_valido, erro_local = await self.validar_local_da_acao(ctx)
            if not local_valido:
                await ctx.send(erro_local)
                return
            ...
            ficha_confirmada = await buscar_ficha(ctx.author.id)
            if not ficha_confirmada or ficha_confirmada["nome"] != ficha["nome"]:
                await ctx.send(
                    "❌ Sua ficha ativa mudou ou foi resetada durante esta ação. "
                    "A narração foi descartada para não misturar personagens."
                )
                return
