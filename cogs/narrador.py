...
class Narrador(commands.Cog):
    ...
    async def gerar_perfil_npc_automatico(self, nome, contexto=""):
        """Reconhece qualquer NPC individual, normaliza o retorno e persiste UMA VEZ."""
        ...
        campos = ("nome", "titulo", "classificacao", "raca", "faccao", "forca",
                  "resistencia", "velocidade", "akuma", "haki", "estilo", "arma",
                  "publico", "combate")

        def normalizar(obj):
            if not isinstance(obj, dict):
                raise ValueError("retorno não é objeto JSON")
            # Aceita pequenas variações de chave sem derrubar um personagem válido.
            aliases = {
                "velocidade_agilidade": "velocidade", "velocidade/agilidade": "velocidade",
                "akuma_no_mi": "akuma", "arma_recurso": "arma", "informacoes": "publico",
                "informações": "publico", "regras_combate": "combate", "classe": "classificacao"
            }
            for origem, destino in aliases.items():
                if destino not in obj and origem in obj:
                    obj[destino] = obj[origem]
            padroes = {
                "nome": str(nome).strip(), "titulo": "Sem título conhecido",
                "classificacao": "Personagem de One Piece", "raca": "Não confirmada",
                "faccao": "Independente", "akuma": "Nenhuma/Não confirmada",
                "haki": "Não confirmado", "estilo": "Não confirmado",
                "arma": "Nenhuma/Não confirmada",
                "publico": "Personagem reconhecido no universo de One Piece.",
                "combate": "Usar somente capacidades canônicas confirmadas e coerentes com a fase do RP."
            }
            for k, v in padroes.items():
                if obj.get(k) in (None, ""):
                    obj[k] = v
            for campo in ("forca", "resistencia", "velocidade"):
                valor = obj.get(campo, 0)
                if isinstance(valor, str):
                    valor = re.sub(r"[^0-9-]", "", valor) or "0"
                obj[campo] = max(0, min(50000, int(valor)))
            return {k: obj[k] for k in campos}

        entrada = f"""
PERSONAGEM CONSULTADO: {nome}
CONTEXTO/Fase do RP: {contexto or 'fase atual do Sea\'s Paradise'}

Identifique o personagem. Se ele for canônico de One Piece, reconheça-o mesmo que seja secundário,
obscuro ou não exista no catálogo local. Use conhecimento canônico e respeite a fase/cronologia.
Se o nome for apelido comum (ex.: Kuma), resolva para o personagem canônico mais provável
(ex.: Bartholomew Kuma). Não invente poderes ainda não confirmados nessa fase.
Se realmente não for possível identificar como personagem canônico, trate como NPC original SOMENTE
quando o contexto do RP indicar que ele existe na cena.

ESCALA Sea's Paradise: 0..50.000 para Força, Resistência e Velocidade/Agilidade.
Responda SOMENTE um objeto JSON. Chaves obrigatórias desejadas:
...
""".strip()
        ...
        for tentativa in range(2):
            resposta = await self.client.responses.create(
                ...
                instructions=(
                    "Você é o catalogador canônico de NPCs do Sea's Paradise, universo One Piece. "
                    "Reconheça personagens por nome, sobrenome ou apelido. Retorne somente JSON válido, "
                    "sem markdown e sem texto antes/depois."
                ),
                input=entrada + ("\nATENÇÃO: a resposta anterior foi inválida. Retorne APENAS o JSON solicitado." if tentativa else ""),
                max_output_tokens=900,
            )
            bruto = (resposta.output_text or "").strip()
            try:
                limpo = re.sub(r"^```(?:json)?\s*|\s*```$", "", bruto, flags=re.I | re.S).strip()
                try:
                    obj = json.loads(limpo)
                except json.JSONDecodeError:
                    achado = re.search(r"\{.*\}", limpo, re.S)
                    if not achado:
                        raise
                    obj = json.loads(achado.group(0))
                perfil = normalizar(obj)
        ...
        raise RuntimeError(f"Não consegui validar o perfil automático de {nome}: {ultimo_erro}")
