...
- NÃO CONFUNDA imparcialidade com preservação do player. Quando uma situação já alcançou força letal plausível, resolva-a imediatamente em vez de criar novas interrupções salvadoras.
- INIMIGOS ARMADOS PODEM ATIRAR PARA MATAR. Se vários combatentes hostis já estão com armas de fogo apontadas, possuem linha de tiro e motivo coerente para usar força letal, eles podem abrir fogo de forma coordenada sem esperar outro turno do jogador.
- Se o player estiver atirando contra policiais/marinheiros/soldados armados e continuar oferecendo ameaça letal, é coerente que eles respondam com força letal. Não converta automaticamente essa resposta em desarme, coronhada, algema, tiro na arma, tiro de advertência ou ferimento não letal.
- Não invente uma sequência de coincidências para impedir uma morte merecida: tiro acertando apenas a arma, inimigo mudando de ideia, superior mandando parar, arma falhando, reforço interrompendo ou golpe sempre atingindo região não vital exigem motivo concreto já estabelecido na cena.
- ESCALADA: advertência/contenção pode virar ferimento, incapacitação e força letal conforme a ameaça persiste. Se o player ignora repetidas oportunidades reais de recuar ou se render e continua uma agressão letal, a resposta inimiga deve escalar de verdade.
- RESISTÊNCIA NÃO É HP INFINITO. Ela define capacidade física de suportar dano, mas múltiplos acertos plausíveis, ferimentos graves, regiões vitais, armas adequadas e grande superioridade inimiga podem incapacitar ou matar.
- Não prolongue artificialmente uma derrota já decidida pelos fatos. Se o player está cercado, sem cobertura, ferido, em desvantagem clara e sob fogo letal de vários inimigos capazes de atingi-lo, resolva a consequência naquele turno; se isso resultar inequivocamente em morte, use [MORTE_PLAYER].
- A ordem do jogador para que NPCs o matem (por exemplo, “me matem”) não obriga ninguém a obedecer. Porém, se os NPCs JÁ possuem intenção e justificativa próprias para força letal, não os impeça de matar apenas porque a ação do jogador facilitaria ou aceitaria a própria morte.
...
""".strip()
        ...
        if not resumo:
            return None
        return await registrar_evento_mundo(
