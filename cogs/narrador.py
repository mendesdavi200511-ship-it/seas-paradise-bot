...
""".strip()
    ...
    async def deve_encerrar_sessao(self, sessao_id, narracao_atual):
...
ÚLTIMA NARRAÇÃO:
{narracao_atual}
A sessão compartilhada terminou de forma inequívoca?
SIM somente se o objetivo/conflito compartilhado acabou e não há ação, transporte, perseguição,
combate ou tarefa imediata pendente para nenhum participante. Um player terminar sua parte não basta.
Responda somente SIM ou NAO."""
    ...
    async def resolver_cena_multiplayer(self,ctx,sessao,acoes):
        ...
        mundo=await self.contexto_mundo(declaracoes,None)
...
Use [ENCERRAR_SESSAO] só se a sessão inteira acabou.
...
Seja objetivo: 2 a 6 parágrafos curtos."""
        ...
        if not auto:
            try: auto=await self.deve_encerrar_sessao(sessao["id"],texto)
        ...
        async with locks[cena_id]:
            ...
            if len(participantes_ciclo)>1:
                ...
                if faltam:
                    ...
                    return
                async with ctx.typing(): await self.resolver_cena_multiplayer(ctx,sessao,declaradas)
                return
            ...
            if not encerrar_sessao_auto and not morte_player:
                try: encerrar_sessao_auto=await self.deve_encerrar_sessao(sessao["id"],narracao)
                except Exception as ex: print(f"⚠️ Verificador de encerramento: {ex}")
