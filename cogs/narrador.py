...
""".strip()
        ...
        texto_fato = f"{acao}\n{narracao}".casefold()
        violencia = any(x in texto_fato for x in ("agred","espanc","chute","soco","ferid","sang","matou","morreu","assassin","roub","furt","explod","destru","atac","tirote","sequestr"))
        publico = any(x in texto_fato for x in ("civil","civis","testemunh","multidão","multidao","público","publico","comerciante","morador","pessoas","pedestre"))
        marinha = any(x in texto_fato for x in ("marinha","marinheiro","marinheiros","patrulha","soldado","oficial"))

        # Fato consumado evidente não pode desaparecer porque a IA classificadora foi conservadora.
        if not dados.get("registrar") and violencia:
            dados = {
                "registrar": True, "tipo": "crime/confronto público" if publico else "confronto",
                "resumo": narracao[:1000], "gravidade": 4 if publico else 3, "alcance": "local",
                "testemunhas": publico or marinha, "marinha_sabe": marinha, "governo_sabe": False,
                "piratas_sabem": False, "publico_sabe": publico, "hostil_marinha": marinha,
                "hostil_governo": False, "hostil_piratas": False, "ajuda_publica": False,
                "virou_noticia": publico and marinha,
                "manchete": f"Incidente envolvendo {ficha['nome']} mobiliza a Marinha em {localizacao or 'área local'}"
            }
        ...
        if dados.get("registrar"):
            ...
            if violencia and publico and marinha:
                dados["virou_noticia"] = True
                if not str(dados.get("manchete") or "").strip():
                    dados["manchete"] = f"Incidente envolvendo {ficha['nome']} mobiliza a Marinha em {localizacao or 'área local'}"
