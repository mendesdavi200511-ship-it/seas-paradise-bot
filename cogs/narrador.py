            ...
            try:
                resposta = await self.client.responses.create(
                    ...
                    instructions=(
                        ...
                        "Todos os campos pedidos são obrigatórios e devem ser específicos do NPC. "
                        "Nunca use perfil provisório, identificação pendente ou valores genéricos. "
                ...
                for campo in ("forca", "resistencia", "velocidade"):
                    candidato[campo] = max(0, min(50000, int(candidato[campo])))
                if not perfil_npc_valido(candidato):
                    raise ValueError("perfil incompleto, provisório ou genérico")
                perfil = candidato
                break
            except (KeyError, TypeError, ValueError, json.JSONDecodeError) as erro:
                ultimo_erro = erro
                entrada += "\n\nA tentativa anterior foi inválida. Gere novamente TODOS os campos com dados específicos e coerentes deste NPC."

        if perfil is None:
            raise RuntimeError(f"Perfil automático inválido para {nome}: {ultimo_erro}")

        # Persiste primeiro; perfil antigo inválido é substituído e perfil válido já existente vence.
