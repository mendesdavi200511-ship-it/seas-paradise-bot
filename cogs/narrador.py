...
from database.database import (
    ...
    registrar_historico_narrador,
    buscar_historico_narrador,
)
...
def chave_cena(ctx):
    # Cada personagem tem sua própria continuidade dentro do canal.
    # Antes a chave era só o canal e players diferentes podiam compartilhar histórico/lock.
...
async def hidratar_historico_persistente(ctx):
    """Restaura os últimos turnos do personagem após reinício do bot ou troca de canal."""
    cena_id = chave_cena(ctx)
    if historicos.get(cena_id):
        return
    rows = await buscar_historico_narrador(ctx.author.id, LIMITE_HISTORICO)
    if not rows:
        return
    historicos[cena_id] = [
        {"acao": r["acao"], "narracao": r["narracao"]}
        for r in rows
    ][-LIMITE_HISTORICO:]
...
- O HISTÓRICO RECENTE é continuidade factual da mesma campanha: não reinicie a cena, não apague ferimentos, perseguições, alertas, posições, ameaças ou consequências ainda não resolvidas.
- Se a ação atual for vaga (ex.: "olhando", "observo", "fico parado"), continue PRIMEIRO a situação pendente do último turno; não substitua por uma nova cena genérica.
...
""".strip()
    ...
    async def acao(self, ctx, *, texto: str):
        ...
        async with locks[cena_id]:
            # Histórico deixa de depender da RAM: reiniciar o bot não reinicia a cena.
            await hidratar_historico_persistente(ctx)
            ...
            # Persiste TODO turno narrado, não apenas acontecimentos considerados "importantes".
            # Isso é o que garante que ações simples como "olhando" continuem a cena real anterior.
            try:
                local_hist = await buscar_localizacao_jogador(ctx.author.id)
                await registrar_historico_narrador(
                    ctx.author.id,
                    ctx.channel.id,
                    texto,
                    narracao,
                    localizacao=(local_hist["localizacao"] if local_hist else None),
                )
            except Exception as erro_hist:
                print(
                    f"⚠️ ERRO AO PERSISTIR HISTÓRICO DO NARRADOR — "
                    f"{type(erro_hist).__name__}: {erro_hist}"
                )
