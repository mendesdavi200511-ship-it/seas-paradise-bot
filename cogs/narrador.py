...
- Se a SESSÃO inteira tiver terminado de forma inequívoca (objetivo/conflito encerrado e não há continuação imediata), termine com [ENCERRAR_SESSAO]. Não use por pausa, silêncio, fim de um único ataque ou porque apenas um jogador saiu.
...
""".strip()
    ...
    async def iniciar_narracao(self, ctx):
        ...
        ok,erro,local_canal,estado=await self.validar_localizacao_do_canal(ctx)
    ...
    async def entrar_narracao(self,ctx):
        ...
        ficha=await buscar_ficha(ctx.author.id)
        if not ficha: await ctx.send("❌ Você ainda não possui ficha ativa."); return
        ok,erro,_,_=await self.validar_localizacao_do_canal(ctx)
        if not ok: await ctx.send(erro); return
        ps=await listar_participantes_sessao(s["id"],True)
        if any(p["user_id"]==ctx.author.id for p in ps): await ctx.send("✅ Você já está na sessão."); return
        ...
        if s["iniciada"]:
            await ctx.send(f"➕ **{ficha['nome']} entrou na narração em andamento.**"); return
        if len(ps)>=qtd:
            await marcar_sessao_iniciada(s["id"])
