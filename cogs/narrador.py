...
Identifique o personagem mesmo se ele não estiver no catálogo local. Se for de One Piece, use somente capacidades confirmadas e coerentes com a fase indicada. Se for personagem conhecido de outra obra/jogo/anime/filme/HQ (por exemplo Scorpion de Mortal Kombat), reconheça sua identidade e adapte suas capacidades à escala do Sea's Paradise sem tratá-lo como canônico de One Piece.
...
""".strip()
        ...
        match = re.search(r"\{.*\}", bruto, re.S)
        if not match:
            # Uma saída vazia da IA não pode quebrar !status.
            bruto = json.dumps({
                "nome": str(nome).strip().title(), "titulo": "Identificação pendente",
                "classificacao": "Perfil provisório", "raca": "Não confirmada",
                "faccao": "Independente", "forca": 1000, "resistencia": 1000,
                "velocidade": 1000, "akuma": "Nenhuma/Não confirmada",
                "haki": "Não confirmado", "estilo": "A confirmar", "arma": "A confirmar",
                "publico": "Perfil provisório; identificação automática pendente.",
                "combate": "Não inventar poderes até identificação confiável."
            }, ensure_ascii=False)
            match = re.search(r"\{.*\}", bruto, re.S)
        perfil = json.loads(match.group(0))

        obrigatorias = ("nome", "titulo", "classificacao", "raca", "faccao", "forca", "resistencia", "velocidade", "akuma", "haki", "estilo", "arma", "publico", "combate")
        if any(k not in perfil for k in obrigatorias):
            raise RuntimeError(f"Perfil automático incompleto para {nome}.")
        for campo in ("forca", "resistencia", "velocidade"):
            perfil[campo] = max(0, min(50000, int(perfil[campo])))
...
""".strip()
        ...
        try:
            ...
            if achado:
                ...
                dados = {}
        texto_fato = f"{acao}\n{narracao}".casefold()
        # Classifica o RESULTADO confirmado. A cena real usa "atinge/cambalear";
        # esses termos não existiam no detector antigo, por isso o evento sumia.
        violencia = any(x in texto_fato for x in (
            "agred","espanc","chute","chuta","chutou","soco","soca","socou","ferid","sang",
            "matou","morreu","assassin","roub","furt","explod","destru","atac","tirote","sequestr",
            "atinge","atingiu","acerta","acertou","golpeia","golpeou","cambalear","cambaleia",
            "derruba","derrubou","cai no chão","cai de lado","quebra","quebrou","disparo","tiro","esfaque"
        ))
        publico = any(x in texto_fato for x in (
            "civil","civis","testemunh","multidão","multidao","público","publico","comerciante",
            "morador","pessoas","pedestre","passantes","tumulto","alarme","chama a atenção","chama a atencao"
        ))
        marinha = any(x in texto_fato for x in (
            "marinha","marinheiro","marinheiros","patrulha","soldado","oficial"
        ))
        governo = any(x in texto_fato for x in (
            "governo mundial","cipher pol","cp0","cp9","dragão celestial","dragao celestial"
        ))
        prisao_ou_cacada = any(x in texto_fato for x in (
            "preso","prisão","prisao","algemado","custódia","custodia","cercado","rota de fuga",
            "largue","mãos onde","maos onde","arma na direção","arma na direcao"
        ))

        # Fato objetivo confirmado pela própria narração não depende da IA classificar certo.
        fato_objetivo = violencia or prisao_ou_cacada
        if fato_objetivo and not dados.get("registrar"):
            dados = {
                "registrar": True,
                "tipo": "agressao_contra_marinha" if (violencia and marinha) else (
                    "agressao_contra_governo" if (violencia and governo) else
                    ("crime_publico" if violencia and publico else "ocorrencia")
                ),
                "resumo": narracao[:1000],
                "gravidade": 6 if (violencia and (marinha or governo)) else (4 if violencia else 2),
                "alcance": "regional" if (violencia and (marinha or governo)) else "local",
                "testemunhas": bool(publico or marinha or governo),
                "marinha_sabe": bool(marinha or prisao_ou_cacada),
                "governo_sabe": bool(governo),
                "piratas_sabem": False,
                "publico_sabe": bool(publico),
                "hostil_marinha": bool(violencia and marinha),
                "hostil_governo": bool(violencia and governo),
                "hostil_piratas": False,
                "ajuda_publica": False,
                "virou_noticia": bool(violencia and publico and (marinha or governo)),
                "manchete": (
                    f"Confronto com a Marinha causa tumulto em {localizacao or 'área local'}"
                    if violencia and marinha else
                    f"Violência pública mobiliza autoridades em {localizacao or 'área local'}"
                )
            }

        if dados.get("registrar"):
            if publico:
                dados["testemunhas"] = True
                dados["publico_sabe"] = True
            if marinha or prisao_ou_cacada:
                dados["marinha_sabe"] = True
            if governo:
                dados["governo_sabe"] = True
            if violencia and marinha:
                dados["hostil_marinha"] = True
                dados["gravidade"] = max(int(dados.get("gravidade") or 1), 6)
                dados["alcance"] = "regional"
            if violencia and governo:
                dados["hostil_governo"] = True
                dados["gravidade"] = max(int(dados.get("gravidade") or 1), 6)
                dados["alcance"] = "regional"
            if violencia and publico and (marinha or governo):
                dados["virou_noticia"] = True
                if not str(dados.get("manchete") or "").strip():
                    dados["manchete"] = (
                        f"Confronto com a Marinha causa tumulto em {localizacao or 'área local'}"
                        if marinha else
                        f"Violência pública mobiliza autoridades em {localizacao or 'área local'}"
        ...
        if not dados.get("registrar"):
            return None
        resumo = str(dados.get("resumo") or "").strip() or narracao[:1000].strip()
        if not resumo:
            return None
